...
/**
 * DOT 소스에 테마 색상 속성을 주입한다.
 * 이미 bgcolor/fillcolor/color 등이 있으면 건드리지 않음 (사용자 커스텀 우선).
 */
function injectGraphvizTheme(code, themeConfig) {
  if (!themeConfig || !themeConfig.colors) return code;

  // 사용자가 이미 색상을 지정한 경우 → 건드리지 않음
  if (/\b(?:fillcolor|bgcolor)\s*=/i.test(code)) return code;
  ...
  // 테마에서 파생된 색상
  const nodeFill = lightenHex(c.primary, 0.75);
  const nodeBorder = '#' + c.primary;
  const nodeText = '#' + c.text;
  const edgeColor = '#' + c.secondary;
  const bgColor = '#FFFFFF';

  // graph 속성 블록을 digraph/graph 선언 바로 뒤에 삽입
  const graphAttrs = [
    `    bgcolor="${bgColor}"`,
    `    fontname="${fontName}"`,
    `    fontsize="12"`,
  ...
  const nodeAttrs = `    node [style="filled" fillcolor="${nodeFill}" color="${nodeBorder}" fontcolor="${nodeText}" fontname="${fontName}" fontsize="11"]`;
  const edgeAttrs = `    edge [color="${edgeColor}" fontcolor="${nodeText}" fontname="${fontName}" fontsize="10"]`;
