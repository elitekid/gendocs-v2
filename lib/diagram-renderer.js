...
function _generateParticipantColors(themeConfig, count) {
  ...
  let sourceHexes;
  if (slots) {
    // accent1, dk2, accent4, accent5, accent3, accent6 순서 (시각적 구분 극대화)
    // accent2(주황)는 후순위 — 참여자 배경으로 부적합할 수 있음
    sourceHexes = [
      slots.accent1, slots.dk2, slots.accent4,
      slots.accent5, slots.accent3, slots.accent6,
      slots.accent2,
    ].filter(Boolean);
  } else if (colors) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // 밝기 0.28~0.42 범위 (흰색 텍스트 가독성)
    let targetL;
    if (l < 0.20) targetL = 0.30;
    else if (l > 0.50) targetL = 0.35;
    else targetL = Math.max(0.25, Math.min(0.42, l));

    // 무채색(grey)은 그대로, 유채색은 충분한 채도 보장
    const targetS = s < 0.05 ? s : Math.max(0.35, Math.min(0.75, s));
 ...
 */
function _recolorSequenceParticipants(svgContent, themeConfig, participantOrder) {
  ...
  // 2. Actor 텍스트를 흰색으로 변경 (CSS 오버라이드)
...
function _mapToThemeColor(hex, familyMap) {
  ...
  // 연한 색상 (l > 0.78)만 치환 — 진한 색상은 사용자 의도가 강함
  if (l > 0.78) {
  ...
  async render(code, outputPath, options) {
    ...
    if (usesSvgPipeline) {
      ...
      if (fs.existsSync(svgPath)) {
        ...
        // 참여자별 개별 색상 적용
        svgContent = _recolorSequenceParticipants(svgContent, options.themeConfig, participants);
