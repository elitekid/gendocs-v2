...
function resolveTheme(config, projectRoot) {
  ...
  const _mergeKeys = ['colors', 'fonts', 'sizes', 'syntax', 'spacing', 'code', 'header', 'footer', 'cover'];
...
async function buildAndSave(config, projectRoot) {
  ...
  const { TableOfContents, Paragraph: DocParagraph, PageBreak: DocPageBreak } = require('docx');
  ...
  if (config.toc) {
    children.push(
      ...
      new DocParagraph({ children: [new DocPageBreak()] })
    );
  }
