...
function resolveTheme(config, projectRoot) {
  ...
  const _mergeKeys = ['colors', 'fonts', 'sizes', 'syntax', 'spacing', 'code', 'header', 'footer', 'cover', 'toc', 'changeHistory'];
...
function _renderTable(lines, i, tableWidthsConfig, totalWidth, docType, t) {
  const tableLines = [];
  while (i < lines.length && lines[i].trim().startsWith('|')) {
  ...
  if (rows.length >= 2) {
    ...
    elems.push(t.createTable(headers, widths, dataRows));
    elems.push(t.spacer(100));
  }
  return { nextIndex: i, elements: elems };
}
...
function convertMarkdownToElements(markdown, config, baseDir, t) {
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith('|')) {
      ...
      const result = _renderTable(lines, i, tableWidthsConfig, totalWidth, docType, t);
      elements.push(...result.elements);
      i = result.nextIndex;
      continue;
    }
...
async function buildAndSave(config, projectRoot) {
  ...
  if (tocConfig && typeof t.createTocPage === 'function') {
    ...
    for (const line of contentCleaned.split('\n')) {
      if (line.startsWith('### ')) headings.push({ level: 3, text: line.substring(4).trim() });
      else if (line.startsWith('## ')) headings.push({ level: 2, text: line.substring(3).trim() });
    }
    if (headings.length > 0) {
      const tocStyle = themeConfig.toc || {};
      children.push(...t.createTocPage(headings, tocStyle));
    }
  }
