...
// ============================================================
// 범용 Markdown → DOCX 요소 변환: 헬퍼 함수
// ============================================================

/** H2 페이지 브레이크 판정 (순수 함수) */
function _decideH2PageBreak(h2Count, title, pbConfig) {
  const { pageBreakH2Set, afterChangeHistory, beforeStopSection } = pbConfig;
  if (pageBreakH2Set.size > 0) return pageBreakH2Set.has(title);
  if (afterChangeHistory && h2Count === 2) return true;
  if (beforeStopSection && h2Count > 2) return true;
  return false;
}

/** H3 페이지 브레이크 판정 (순수 함수) */
function _decideH3PageBreak(opts) {
  const { inNoBreakList, inBreakList, isChangeDetail, changeDetailH3Break,
          hasImage, imageH3AlwaysBreak, defaultH3Break, beforeStopSection,
          isFirstH3AfterH2, lastH2Broke } = opts;
  if (inNoBreakList) return false;
  if (inBreakList) return !isFirstH3AfterH2;
  if (isChangeDetail && changeDetailH3Break) return !isFirstH3AfterH2;
  if (isChangeDetail && !changeDetailH3Break) return false;
  if (hasImage && imageH3AlwaysBreak) return !isFirstH3AfterH2 || !lastH2Broke;
  if (defaultH3Break && beforeStopSection && !isFirstH3AfterH2) return true;
  return false;
}

/** 이미지 섹션 처리 (sectionMap 기반) — { nextIndex, elements } 반환 */
function _renderImageSection(lines, i, imgInfo, imageBasePath, t) {
  const elems = [];
  i++;
  while (i < lines.length) {
    ...
    elems.push(t.text(nextLine.trim()));
    i++;
  }
  ...
  if (fs.existsSync(imagePath)) {
    elems.push(t.createImage(imagePath, imgWidth, imgHeight));
  } else {
  ...
  return { nextIndex: i, elements: elems };
}

/** 블록인용 처리 — { nextIndex, elements } 반환 */
function _renderBlockquote(lines, i, t) {
  ...
  const box = (quoteText.startsWith('주의') || quoteText.startsWith('중요'))
    ? t.warningBox(quoteText) : t.infoBox(quoteText);
  return { nextIndex: i, elements: [box, t.spacer(150)] };
}

/** 코드블록 처리 — { nextIndex, elements } 반환 */
function _renderCodeBlock(lines, i, t) {
  ...
  if (codeLines.length > 0) {
    if (langTag === 'combined' && typeof t.createColoredMessage === 'function') {
      elems.push(t.createColoredMessage(codeLines));
    } else if (langTag === 'signature' && typeof t.createPlainCodeBlock === 'function') {
      elems.push(t.createPlainCodeBlock(codeLines));
    } else {
      ...
      if ((firstNonEmpty.startsWith('{') || firstNonEmpty.startsWith('[')) && typeof t.createJsonBlock === 'function') {
        elems.push(t.createJsonBlock(codeLines));
      } else if (typeof t.createSyntaxCodeBlock === 'function') {
        elems.push(t.createSyntaxCodeBlock(codeLines));
      } else {
        elems.push(t.createCodeBlock(codeLines));
      }
    }
    elems.push(t.spacer(100));
  }
  return { nextIndex: i, elements: elems };
}

/** 테이블 처리 — { nextIndex, elements } 반환 */
function _renderTable(lines, i, tableWidthsConfig, totalWidth, docType, t, tableOptions = {}) {
  const tableLines = [];
  while (i < lines.length && lines[i].trim().startsWith('|')) {
  ...
  if (rows.length >= 2) {
    ...
    elems.push(t.createTable(headers, widths, dataRows, tableOptions));
    elems.push(t.spacer(100));
  }
  return { nextIndex: i, elements: elems };
}

/** 흐름 박스 처리 — { nextIndex, elements } 반환 */
function _renderFlowBox(lines, i, t) {
  const flowLines = [];
  i++;
  while (i < lines.length) {
  ...
  if (flowLines.length > 0) {
    if (typeof t.flowBox === 'function') {
      elems.push(t.flowBox(flowLines));
    } else {
      flowLines.forEach(line => elems.push(t.bullet(line)));
    }
    elems.push(t.spacer(80));
  }
  return { nextIndex: i, elements: elems };
}

// ============================================================
// 범용 Markdown → DOCX 요소 변환: 메인 함수
// ============================================================
...
function convertMarkdownToElements(markdown, config, baseDir, t) {
  ...
  const elements = [];
  let i = 0;
  // 가용 폭: 페이지 폭 - 좌우 마진 (pageMargin + orientation은 resolvedTheme에서)
  const _rt = config._resolvedTheme || {};
  const _orient = config.orientation || _rt.orientation || 'landscape';
  const _pm = _rt.pageMargin || {};
  const _pageW = (_orient === 'portrait') ? 12240 : 15840;
  const _defaultMarginLR = 1440;
  const totalWidth = _pageW - (_pm.left || _defaultMarginLR) - (_pm.right || _defaultMarginLR);
  ...
  let lastH2Broke = false;  // 직전 H2가 실제 break를 했는지
  let currentImageSection = null;
  let beforeStopSection = true;  // h2BreakBeforeSection 이전인지
  let isChangeHistorySection = false;  // 변경이력 섹션 감지
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      ...
      // 변경이력 섹션 감지 (제목에 "변경" 또는 "개정" 포함)
      isChangeHistorySection = /변경|개정/.test(title);
      ...
      const doBreak = _decideH2PageBreak(h2Count, title, { pageBreakH2Set, afterChangeHistory, beforeStopSection });
      if (doBreak) elements.push(t.pageBreak());
    ...
    if (line.startsWith('### ')) {
      ...
      // 명시적 break 목록 (섹션 번호 또는 전체 제목 매칭)
      const inBreakList = pageBreakH3Set.has(title) || (sectionNum && pageBreakH3Set.has(sectionNum));
      const inNoBreakList = noBreakH3Set.has(title) || (sectionNum && noBreakH3Set.has(sectionNum));

      const doBreak = _decideH3PageBreak({
        inNoBreakList, inBreakList, isChangeDetail, changeDetailH3Break,
        hasImage, imageH3AlwaysBreak, defaultH3Break, beforeStopSection,
        isFirstH3AfterH2, lastH2Broke
      });
      if (doBreak) elements.push(t.pageBreak());
      ...
      if (hasImageConfig) {
        ...
        const result = _renderImageSection(lines, i, imageSectionMap[sectionNum], imageBasePath, t);
    ...
    // > 인용문 (blockquote)
    if (line.startsWith('> ')) {
      const result = _renderBlockquote(lines, i, t);
      elements.push(...result.elements);
      i = result.nextIndex;
      continue;
    }
    ...
    if (line.trim().match(/^!\[.*?\]\(.+?\)$/)) {
      ...
      if (imgMatch && t.createImage) {
        ...
        if (fs.existsSync(resolvedPath)) {
          const maxWidthPx = (config.orientation === 'portrait') ? 560 : 780;
          const diagramMaxHeight = config.diagrams && config.diagrams.maxHeight;
          const dims = getImageDimensions(resolvedPath, maxWidthPx, diagramMaxHeight || 500);
    ...
    if (line.trim().startsWith('```')) {
      const result = _renderCodeBlock(lines, i, t);
      elements.push(...result.elements);
      i = result.nextIndex;
      continue;
    }
    ...
    if (line.trim().startsWith('|')) {
      ...
      const tableOpts = {};
      if (isChangeHistorySection && config._resolvedTheme?.changeHistory?.totalRows) {
        tableOpts.minRows = config._resolvedTheme.changeHistory.totalRows;
      }
      const result = _renderTable(lines, i, tableWidthsConfig, totalWidth, docType, t, tableOpts);
      elements.push(...result.elements);
      i = result.nextIndex;
      continue;
    }
    ...
    if (numMatch) {
      if (typeof t.numberedItem === 'function') {
        elements.push(t.numberedItem(numMatch[1], numMatch[2]));
      } else {
        elements.push(t.bullet(numMatch[2]));
      }
      i++; continue;
    }
    ...
    if (flowLabelMatch) {
      const result = _renderFlowBox(lines, i, t);
      elements.push(...result.elements);
      i = result.nextIndex;
      continue;
    }
    ...
    // 일반 텍스트
    if (line.trim()) {
      elements.push(t.text(line.trim()));
    }
    i++;
  }
...
function createCoverPage(docInfo, baseDir, t, logoPath) {
  ...
  const projectInfo = [
    docInfo.version && { label: '버전', value: docInfo.version },
    docInfo.modifiedDate && { label: '최종 수정일', value: docInfo.modifiedDate }
  ].filter(Boolean);
...
async function buildAndSave(config, projectRoot) {
  ...
  const coverElements = createCoverPage(docInfo, baseDir, t, logoPath);
  const children = [...coverElements];

  // 정적 목차 (toc 옵션 또는 styleProfile에서 감지)
  const tocConfig = config.toc || (themeConfig.toc && themeConfig.toc.enabled);
  let tocElements = [];
  if (tocConfig && typeof t.createTocPage === 'function') {
    const tocStyle = themeConfig.toc || {};
    const tocExclude = new Set(tocStyle.exclude || []);
    const headings = [];
    for (const line of contentCleaned.split('\n')) {
      let heading = null;
      if (line.startsWith('### ')) heading = { level: 3, text: line.substring(4).trim() };
      else if (line.startsWith('## ')) heading = { level: 2, text: line.substring(3).trim() };
      if (heading && !tocExclude.has(heading.text)) {
        headings.push(heading);
      }
    }
    if (headings.length > 0) {
      tocElements = t.createTocPage(headings, tocStyle);
    }
  }

  // TOC 위치: afterChangeHistory → 변경이력 뒤에 삽입, 기본값 → 표지 바로 뒤
  const tocPosition = (themeConfig.toc && themeConfig.toc.position) || 'afterCover';
  if (tocPosition === 'afterChangeHistory') {
    // 두 번째 Heading2 직전에서 분리 (변경이력 = 첫 번째 H2 섹션)
    let splitAt = -1;
    let h2Found = 0;
    for (let ei = 0; ei < contentElements.length; ei++) {
      const el = contentElements[ei];
      const pPr = el.properties;
      if (pPr && pPr.root) {
        for (const child of pPr.root) {
          if (child.rootKey === 'w:pStyle' && child.root) {
            for (const attr of child.root) {
              if (attr.rootKey === '_attr' && attr.root && attr.root.val === 'Heading2') {
                h2Found++;
                if (h2Found === 2) { splitAt = ei; break; }
              }
            }
          }
          if (splitAt >= 0) break;
    ...
    if (splitAt > 0) {
      const changeHistoryPart = contentElements.slice(0, splitAt);
      const restPart = contentElements.slice(splitAt);
      children.push(...changeHistoryPart, ...tocElements, ...restPart);
    } else {
      children.push(...tocElements, ...contentElements);
    }
  } else {
    children.push(...tocElements, ...contentElements);
  }
