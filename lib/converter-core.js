...
async function buildAndSave(config, projectRoot) {
  ...
  if (ext === '.pdf') {
    // PDF → 플레인 DOCX (템플릿/테마 우회)
    const pdfParser = require('./parsers/pdf-parser');
    const { buildPlainDocx } = require('./ir/plain-docx');
    const imageDir = path.join(path.dirname(outputPath), '.images');
    const pdfOpts = { baseDir, imageDir };
    if (config.pdfClassify) pdfOpts.classify = config.pdfClassify;
    const ir = pdfParser.parse(sourcePath, pdfOpts);
    await buildPlainDocx(ir, outputPath);
    return { outputPath };
  } else {
