...
// Word가 요소 사이에 추가하는 고정 간격 (측정값)
const WORD_GAP = {
  headingToTable: 3.9,    // heading → table
  tableToHeading: 3.3,    // table → heading
  headingToHeading: 7.1,  // heading → heading
  paragraphToTable: 3.9,  // paragraph → table
  tableToParagraph: 3.3,  // table → paragraph
};

function renderHeading(node, skipSpacing = false, prevType = null) {
  const s = node.style || {};
  // Word 추가 간격 보정: 이전 요소 유형에 따라 spacingBefore에서 차감
  let beforePt = node.spacingBefore != null ? node.spacingBefore : 12;
  if (!skipSpacing && node.spacingBefore != null) {
    if (prevType === 'table') {
      beforePt = Math.max(0, beforePt - WORD_GAP.tableToHeading);
    } else if (prevType === 'heading' || prevType === 'paragraph') {
      beforePt = Math.max(0, beforePt - WORD_GAP.headingToHeading);
    }
  }
...
function renderCodeBlock(node) {
  ...
  // 코드블록 줄 간격: IR lineSpacing에 Word 보정 적용
  // Word는 line spacing을 실제보다 ~19% 크게 렌더링 (font descent/gap 추가)
  // 보정 계수 0.84 = 28.3(원본) / 33.7(Word 렌더링)
  const CODE_LINE_SPACING_FACTOR = 0.84;
  const codeLineSpacing = s.lineSpacing
    ? Math.round(s.lineSpacing * CODE_LINE_SPACING_FACTOR * 20)  // pt → 보정 → twips
    : 276;  // 기본 1.15배
...
/**
 * Word 폰트 메트릭 기반 텍스트 폭 추정 (pt).
 * PDF와 Word의 Gulim 등 폰트 렌더링 차이를 반영.
 */
function estimateTextWidth(text, sizePt) {
  let w = 0;
 ...
 */
function adjustColumnWidths(columns, rows, sizePt) {
  ...
  const maxTextWidths = columns.map((col, ci) => {
    let maxW = estimateTextWidth(col.header || '', sizePt);
    for (const row of rows) {
      if (!row[ci]) continue;
      const cellText = (row[ci].runs || []).map(r => r.text).join('');
      // 줄바꿈이 있으면 각 줄 중 가장 긴 것
      for (const line of cellText.split('\n')) {
        const tw = estimateTextWidth(line, sizePt);
        if (tw > maxW) maxW = tw;
      }
    }
    return maxW;
...
function renderTable(node) {
  ...
  const adjustedWidths = adjustColumnWidths(columns, rows, tableSizePt);
  ...
  const dataRows = rows.map((row, rowIdx) => {
    ...
    if (rh) {
      // 멀티라인 셀이 있으면 ATLEAST (Word가 늘려줌), 아니면 EXACT
      const hasMultiLine = row.some((cell) => {
        const text = (cell.runs || []).map(r => r.text).join('');
        return text.includes('\n');
      });
      // 컬럼 너비 대비 텍스트 오버플로우도 체크 (줄바꿈 없어도 넘치면 2줄)
      const hasOverflow = row.some((cell, ci) => {
        const text = (cell.runs || []).map(r => r.text).join('');
        const tw = estimateTextWidth(text, tableSizePt);
...
async function buildPlainDocx(ir, outputPath) {
  ...
  for (const node of content) {
    ...
    switch (node.type) {
      case 'heading':
        children.push(renderHeading(node, currentSkip, prevType));
        break;
      case 'paragraph':
