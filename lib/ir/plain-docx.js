...
function renderHeading(node, skipSpacing = false, prevType = null, prevStyle = null) {
  ...
  // line=9pt에서 paragraph→table gap이 2.1pt (원본 4.1pt) → after로 2pt 보상
  return new Paragraph({
    spacing: {
      before: skipSpacing ? 0 : ptToDxa(rawBefore),
      after: ptToDxa(2),
...
function renderCodeBlock(node) {
  ...
  // spacingBefore가 없으면 codeBefore를 첫 줄에도 적용 (Example→{ gap 일치)
  // 총 높이 유지: 첫 줄 before 추가분만큼 나머지 줄 before 감소
  // 첫 줄 before = codeBefore * 0.6 (Example→{ gap과 나머지 줄 gap의 균형)
  const firstLineBefore = node.spacingBefore ? ptToDxa(node.spacingBefore) : Math.round(codeBefore * 0.6);
