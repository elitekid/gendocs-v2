...
// Word AUTO line rule에서 실제 렌더링 = 지정값 / 0.84 (pdf2docx 동일 방식)
...
function calcWordGap(prevType, prevFont, prevSize, nextFont, nextSize, prevStyle) {
  ...
  // paragraph/heading → heading: Word는 인접 paragraph의 여백 중 큰 쪽을 적용 (collapse)
...
function renderHeading(node, skipSpacing = false, prevType = null, prevStyle = null, nextNode = null) {
  ...
  // 다음이 table이고 spacingBefore가 크면, heading before→after로 재분배 (총 높이 변화 없음)
  let beforePt = skipSpacing ? 0 : rawBefore;
  // after: 다음 요소에 spacingBefore 없으면 heading→body gap 보상 (약 5pt)
...
function renderParagraph(node, lineSpacing) {
  ...
  // dot leader (TOC 스타일): 텍스트 + tab(dot leader) → right margin
  if (node._dotLeader) {
    // tab position은 section left margin 기준 상대값 (page 절대좌표 - left margin)
...
function renderCodeBlock(node) {
  ...
  // pdf2docx 방식: line=~9pt (최소 line box), before=줄간격-렌더링높이
  const codeSizePt = s.size || 9;
  const lineGap = s.lineSpacing || 0;
  // before = PDF 줄 간격 - 실제 렌더링 줄 높이 (Word에서 font에 맞게 확장한 높이)
  // 렌더링 줄 높이 ≈ fontSize * 1.19 (AUTO에서 9pt line → ~11pt rendered for 11pt font)
  // Word가 코드 줄을 fontSize+0.6pt로 렌더링하므로 before에서 0.6pt 보상
  ...
  // 첫 줄: spacingBefore(PDF 실측)가 있으면 사용, 없으면 codeBefore * 0.5 (균형)
  // 총 높이 유지: 첫 줄 before 추가분만큼 나머지 줄 before 감소
  ...
  return (node.lines || []).map((line, lineIdx) => {
    ...
    const pOpts = {
      ...
      children: [new TextRun({
        text: (line || ' ').trimStart(),  // 기존 공백 제거 (indent로 대체)
        font,
        size,
        color,
      })],
...
function renderTable(node) {
  ...
  const dataRows = rows.map((row, rowIdx) => {
    ...
    if (rh) {
      ...
      if (!hasMultiLine) {
        // multiline은 위에서 처리됨, overflow만 여기서
  ...
  // _noHeader: cross-page fragment — 헤더 행 없이 데이터 행만 렌더링
...
/**
 * PDF header/footer 정보 → Word Header/Footer Paragraph 생성.
 * 좌/중/우 정렬 항목을 탭스톱으로 배치.
 * "__PAGE__" → Word 자동 페이지 번호 필드.
 */
function _buildHeaderFooterParagraph(items, meta) {
...
async function buildPlainDocx(ir, outputPath) {
  ...
  for (const [pageNum, pageNodes] of pages) {
    ...
    for (const node of pageNodes) {
      // pdf2docx 모델: line=9pt에서 Word 추가분 없음 → before 보정 불필요

      const skipSpacing = prevType === null;  // section 첫 요소는 spacing skip
    ...
    if (children.length > 0) {
      ...
      // header/footer (meta에서 추출된 반복 텍스트) — section 레벨에 배치
      // 첫 페이지(표지)는 header 없이 footer만 적용
      const isFirstSection = sections.length === 0;
      if (meta.header && meta.header.length > 0 && !isFirstSection) {
        const hdrChildren = _buildHeaderFooterParagraph(meta.header, meta);
        section.headers = { default: new Header({ children: hdrChildren }) };
      }
      if (meta.footer && meta.footer.length > 0) {
        const ftrChildren = _buildHeaderFooterParagraph(meta.footer, meta);
