...
function renderImage(node) {
  ...
  // 이미지가 페이지 가용 높이의 80%를 넘으면 축소
  const maxImageHeight = 380;  // pt (가용 ~457pt에서 제목/간격 ~77pt 제외)
  let imgW = node.width || 400;
  let imgH = node.height || 300;
  if (imgH > maxImageHeight) {
    const scale = maxImageHeight / imgH;
    imgW = imgW * scale;
    imgH = maxImageHeight;
  }
  const width = Math.round(ptToPx(imgW));
  const height = Math.round(ptToPx(imgH));
