...
function renderParagraph(node, lineSpacing) {
  ...
  return new Paragraph({
    spacing: {
      before: node._adjustedSpacingBefore != null ? ptToDxa(node._adjustedSpacingBefore)
...
function renderCodeBlock(node) {
  ...
  // pdf2docx 방식: line = fontSize*0.84 (줄 높이만), before = 줄 간격
  const codeSizePt = s.size || 9;
  const codeLine = Math.round(codeSizePt * WORD_LINE_SPACING_FACTOR * 20);
  const lineGap = s.lineSpacing || 0;  // PDF에서 측정한 y좌표 차이
  // before = 줄간격 - 줄높이 (줄높이 = fontSize, Word가 렌더링하면 fontSize로 맞춰줌)
  const codeBefore = lineGap > codeSizePt ? Math.round((lineGap - codeSizePt) * 20) : 0;

  return (node.lines || []).map((line, lineIdx) => {
    const pOpts = {
      spacing: { before: lineIdx === 0 ? 0 : codeBefore, after: 0, line: codeLine, lineRule: LineRuleType.AUTO },
      children: [new TextRun({
        text: line || ' ',
        font,
        size,
...
function renderImage(node, prevType, prevStyle) {
  ...
  // spacingBefore: 통합 보정값 사용
  const imgBefore = node._adjustedSpacingBefore != null ? node._adjustedSpacingBefore
    : node.spacingBefore || 0;
...
async function buildPlainDocx(ir, outputPath) {
  ...
  for (const [pageNum, pageNodes] of pages) {
    ...
    for (const node of pageNodes) {
      // spacingBefore 보정: 모든 요소에 동적 적용
      if (node.spacingBefore != null && prevType && node.type !== 'heading') {
        const pFont = prevStyle?.font || '_default';
        const pSize = prevStyle?.size || 11;
        const nFont = (node.style || node.runs?.[0])?.font || '_default';
        const nSize = (node.style || node.runs?.[0])?.size || 11;
        const wordGap = calcWordGap(prevType, pFont, pSize, nFont, nSize, prevStyle);
        node._adjustedSpacingBefore = Math.max(0, node.spacingBefore - wordGap);
      }
      ...
      switch (node.type) {
        case 'heading':
          children.push(renderHeading(node, false, prevType, prevStyle));
          break;
        case 'paragraph':
    ...
    if (children.length > 0) {
      sections.push({ properties: pageSettings, children });
    }
  }
